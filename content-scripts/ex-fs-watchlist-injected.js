(function() {
    ...
    async function loadFirebaseScripts() {
        ...
        try {
            ...
            if (!urlsAttr) {
                throw new Error('Script URLs not found in config');
            }
            
            const scriptUrls = JSON.parse(urlsAttr);
            console.log('[MovieList Extension] Loading Firebase scripts in page context:', scriptUrls.length);
            ...
            for (const url of scriptUrls) {
                await new Promise((resolve, reject) => {
                    ...
                    script.src = url;
                    script.onload = () => {
                        console.log('[MovieList Extension] Loaded script:', url);
                        resolve();
                    };
                    script.onerror = () => {
                        console.error('[MovieList Extension] Failed to load script:', url);
            ...
            if (typeof FirebaseManager === 'undefined') {
                throw new Error('FirebaseManager class not found after loading scripts');
            }
            ...
            if (!firebaseManagerInstance.isInitialized) {
                throw new Error('Firebase Manager initialization timeout');
            }
            
            console.log('[MovieList Extension] Firebase Manager initialized successfully in page context');
            servicesLoading = false;
            return firebaseManagerInstance;
        } catch (error) {
            console.error('[MovieList Extension] Error loading Firebase scripts:', error);
            servicesLoading = false;
            throw error;
        }
    }
    ...
    async function init() {
        console.log('[MovieList Extension] Initializing watchlist feature...');
    ...
    function extractKinopoiskId() {
        ...
        for (const script of scripts) {
            if (script.textContent) {
                const match = script.textContent.match(/kinopoiskID["\s:=]+(\d+)/i) || 
    ...
    async function setupWatchlistButton() {
        ...
        console.log('[MovieList Extension] Setting up watchlist button...');
        
        try {
            const poster = document.querySelector('.FullstoryFormLeft img');
            if (!poster) {
                console.log('[MovieList Extension] Poster not found');
                isSettingUpButton = false;
                return;
            }
            
            // Remove existing button if it exists anywhere
            ...
            // Get the poster container (FullstoryFormLeft)
            const posterContainer = poster.parentElement;
            if (!posterContainer) {
                console.error('[MovieList Extension] Could not find poster container');
                return;
            }
            ...
            // Check if there's already a wrapper around the image
            // If the image's parent is the container, we need to wrap it
            ...
            if (needsWrapper) {
                ...
                // Find all siblings after the image (rating, info, etc.)
                const nextSibling = poster.nextSibling;
                
                // Insert wrapper right after the container starts (before image)
                posterContainer.insertBefore(imageWrapper, poster);
                
                // Move the image into the wrapper
                imageWrapper.appendChild(poster);
            } else {
                // Use existing wrapper, but ensure it has relative positioning
            ...
            // Add button directly to the image wrapper (will overlay the image)
    ...
    function setupMutationObserver() {
        console.log('[MovieList Extension] Setting up mutation observer...');
        
        let debounceTimer = null;
        
        const observer = new MutationObserver((mutations) => {
            // Ignore mutations caused by our own button or wrapper
            const hasOurChanges = mutations.some(mutation => {
                return Array.from(mutation.addedNodes).some(node => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        return node.id === 'movieListWatchlistBtn' || 
                               node.classList?.contains('movieListPosterWrapper') ||

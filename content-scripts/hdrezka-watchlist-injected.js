(function() {
    console.log('[MovieList Extension] Injected script executing in page context for hdrezka.co');
    
    let isSettingUpButton = false;
    
    async function init() {
        console.log('[MovieList Extension] Initializing watchlist feature for hdrezka.co...');
    ...
    function extractMovieTitle() {
        ...
        // Try to get original title first (usually English title on hdrezka.co)
        const originalTitle = document.querySelector('.b-post__origtitle');
        if (originalTitle) {
            let title = originalTitle.textContent.trim();
            // Remove "/ Tron 3" type suffixes
            if (title.includes(' / ')) {
        ...
        const titleElement = document.querySelector('h1[itemprop="name"], .b-post__title h1, h1');
        if (titleElement) {
    ...
    function extractKinopoiskId() {
        ...
        // Look for Kinopoisk link in page info (hdrezka.co uses base64-encoded URLs in /help/ links)
        const kinopoiskLinks = document.querySelectorAll('a[href*="kinopoisk"], a[href*="kp"], a[href*="/help/"]');
        for (const link of kinopoiskLinks) {
            const href = link.getAttribute('href');
            if (!href) continue;
            
            // Check if it's a /help/ link (base64 encoded)
            if (href.includes('/help/')) {
                try {
                    ...
                    if (base64Match && base64Match[1]) {
                        ...
                        const match = urlDecoded.match(/kinopoisk\.ru\/film\/(\d+)/i);
                        if (match && match[1]) {
                            const kpId = parseInt(match[1]);
                            console.log(`[MovieList Extension] Found Kinopoisk ID in base64 link: ${kpId} (decoded from: ${base64String})`);
                            return kpId;
                        }
                    }
                } catch (e) {
                    console.log('[MovieList Extension] Error decoding base64 link:', e);
                }
            }
        ...
        // Look in scripts for Kinopoisk ID
        const scripts = document.querySelectorAll('script');
        for (const script of scripts) {
            if (script.textContent) {
                // Look for base64 encoded Kinopoisk URLs
                const base64Match = script.textContent.match(/aHR0cHMlM0ElMkYlMkZ3d3cua2lub3BvaXNr[^"'\s]+/);
                if (base64Match) {
                    try {
                ...
                // Look for direct Kinopoisk URLs in scripts
    ...
    async function setupWatchlistButton() {
        ...
        console.log('[MovieList Extension] Setting up watchlist button for hdrezka.co...');
        
        try {
            // Find poster container - on hdrezka.co it's .b-sidecover
            const posterContainer = document.querySelector('.b-sidecover');
            if (!posterContainer) {
    ...
    function setupMutationObserver() {
        console.log('[MovieList Extension] Setting up mutation observer for hdrezka.co...');
